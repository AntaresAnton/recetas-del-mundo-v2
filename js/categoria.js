document.addEventListener("DOMContentLoaded", function () {
  ...
  function cargarRecetas(categoria) {
    const recetasUrl =
      recetasUrlBase + (categoria ? "categoria/" + categoria : ""); // Si no se proporciona categoría, se cargan todas las recetas
    if (categoria != "") {
        ...
        if (data.ok) {
          if (Array.isArray(data.data) && data.data.length > 0) {
            data.data.forEach((receta, index) => {
              ...
              recetaElement.innerHTML = `
                            <div class="card mb-4 z-index-b">
                                <img src="${
                                  receta.url_imagen
                                }" class="card-img-top img-thumbnail receta-img" alt="${
                receta.nombre
              }">
                                <div class="card-body">
            ...
            nombreCategoria.textContent = categoria
              ? categoria
              : "Todas las Recetas"; // Actualizar el título con el nombre de la categoría o "Todas las Recetas" si no se proporciona categoría
          } else {
  ...
  function obtenerCategoriaPredeterminada() {
    return fetch(categoriasUrl)
      ...
      .catch((error) => {
        console.error(
          `Error al obtener la categoría predeterminada: ${error.message}`
        );
