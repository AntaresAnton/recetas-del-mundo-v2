...
document.addEventListener("DOMContentLoaded", function () {
  ...
  async function cargarRecetaAleatoria() {
    ...
    <div class="row g-4">
        <!-- Fixed Image Column -->
        <div class="col-md-6 position-sticky" style="top: 0; height: fit-content;">
            <img src="${receta.url_imagen}" 
                 alt="${receta.nombre}"
...
document.addEventListener("DOMContentLoaded", function () {
  if (
    ...
    const recetaDelDiaDescripcion = document.getElementById(
      "recetaDelDiaDescripcion"
    );
    ...
    function inicializarRecetaDelDia() {
      ...
      if (
        recetaGuardada &&
        esMismaFecha(new Date(recetaGuardada.fecha), new Date())
      ) {
    ...
    function updateSidebarWithUserInfo() {
      ...
      if (userInfoString) {
        ...
        if (userActive) {
          ...
          userActive.className =
            userInfo.activo == 1 ? "badge bg-success" : "badge bg-secondary";
        }
      }
    }
...
function loadRecetaAleatoria(data) {
  // Actualizar imagen
  document.getElementById("recetaImg").src = data.imagen;

  // Actualizar título
  document.getElementById("recetaTitulo").textContent = data.nombre;

  // Cargar ingredientes
  const ingredientesList = document.getElementById("ingredientesList");
  ingredientesList.innerHTML = data.ingredientes
    .map(
      (ingrediente) => `
  ...
  instruccionesList.innerHTML = data.instrucciones
    .map(
      (paso, index) => `
